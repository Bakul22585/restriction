...
@Component({
  selector: 'app-expence-preview',
  templateUrl: './expence-preview.component.html',
  styleUrls: ['./expence-preview.component.css']
})
export class ExpencePreviewComponent implements OnInit {
  ...
  fundsResult: ExpencepayModel[] = [];
  fund: ExpencepayModel;
  allfundpay: any = [];
  Mainfund: ExpenceModel;
  UserData: any = [];
  ExpenseUserData: any = [];
  SerialNumber = 1;
  ...
  constructor(
    ...
    public dialogRef: MatDialogRef<ExpencePreviewComponent>,
  ...
  ngOnInit() {
    ...
    // First load
    this.dataSource.loadExpencepay(queryParams, this.fund.id);
    this.dataSource.entitySubject.subscribe(res => {
      this.fundsResult = res;
      this.ExpenseUserData = res[0];
    });
  }
  ...
  ClearCheque(user) {
    const _title: string = this.translate.instant('EXPENCE.CONFIRM_EXPENCE_SIMPLE.TITLE');
    const _description: string = this.translate.instant('EXPENCE.CONFIRM_EXPENCE_SIMPLE.DESCRIPTION');
    const _waitDesciption: string = this.translate.instant('EXPENCE.CONFIRM_EXPENCE_SIMPLE.WAIT_DESCRIPTION');
    const _confirmMessage = this.translate.instant('EXPENCE.CONFIRM_EXPENCE_SIMPLE.MESSAGE');
    ...
    dialogRef.afterClosed().subscribe(confirm => {
      ...
      this.expenceService.CheckExpencepayStatus(user.expence_id).subscribe(res => {
        this.allfundpay = res['data'];
        const _fundpay = new ExpencepayModel();
        _fundpay.id = user.id;
        _fundpay.expence_id = user.expence_id;
        ...
        this.expenceService.updateExpencepay(_fundpay).subscribe(payres => {
          // if (this.allfundpay.length === 1) {
            ...
            // console.log(_fund);
            this.expenceService.updateExpence(_fund).subscribe(responce => {
              this.loadExpencesList();
            });
          // }
          this.loadExpencesList();
        ...
        _history.description = JSON.parse(sessionStorage.getItem('user_data'))._value.username +
          ' was approve cheque expence amount ' + user.paid_amount + ' for ' + this.Mainfund.event_id + ' cheque no ' + user.cheque_no + '.';
  ...
  deleteFund(_item: ExpencepayModel) {
    ...
    dialogRef.afterClosed().subscribe(res => {
      ...
      const obj = {};
      obj['id'] = _item.id;
      obj['expence_id'] = this.fund.id;
      obj['user_id'] = this.fund.user_id;
      this.expenceService.deleteExpencepay(obj).subscribe(() => {
